<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Edit Prescription') }} - #{{ $prescription->id }}</h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form action="{{ route('prescriptions.update', $prescription) }}" method="POST" x-data="prescriptionForm()">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="mb-4">
                                ...
                                <select id="customer_id" name="customer_id" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                                    ...
                                    @foreach($customers as $id => $name)
                                        <option value="{{ $id }}" {{ $prescription->customer_id == $id ? 'selected' : '' }}>{{ $name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-4">
                                ...
                                <x-text-input id="doctor_name" name="doctor_name" type="text" class="mt-1 block w-full" :value="old('doctor_name', $prescription->doctor_name)" required />
                                <x-input-error :messages="$errors->get('doctor_name')" class="mt-2" />
                            </div>

                            <div class="mb-4">
                                <x-input-label for="hospital" :value="__('Hospital / Clinic')" />
                                <x-text-input id="hospital" name="hospital" type="text" class="mt-1 block w-full" :value="old('hospital', $prescription->hospital)" />
                        ...
                        <div class="mb-4">
                            ...
                            <textarea id="diagnosis" name="diagnosis" rows="3" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('diagnosis', $prescription->diagnosis) }}</textarea>
                            <x-input-error :messages="$errors->get('diagnosis')" class="mt-2" />
                        </div>

                        <div class="mb-4">
                            <x-input-label for="notes" :value="__('Notes')" />
                            <textarea id="notes" name="notes" rows="2" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('notes', $prescription->notes) }}</textarea>
                        ...
                        <div class="flex items-center gap-4">
                            <x-primary-button>{{ __('Update Prescription') }}</x-primary-button>
