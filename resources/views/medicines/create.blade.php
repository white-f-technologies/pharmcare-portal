    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-2xl border border-gray-100">
                <div class="p-6">
                    <form action="{{ route('medicines.store') }}" method="POST" enctype="multipart/form-data" x-data="{
                        units: [],
                        addUnit() {
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div class="mb-4">
                                ...
                                <x-text-input id="generic_name" name="generic_name" type="text" class="mt-1 block w-full" :value="old('generic_name')" />
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4 bg-slate-50 p-4 rounded-xl border border-slate-200">
                            <div>
                                ...
                                <select id="base_unit" name="base_unit" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm font-bold text-gray-800">
                                    <option value="Tablet" {{ old('base_unit', 'Tablet') == 'Tablet' ? 'selected' : '' }}>Tablet</option>
                                    <option value="Capsule" {{ old('base_unit') == 'Capsule' ? 'selected' : '' }}>Capsule</option>
                                    <option value="Bottle" {{ old('base_unit') == 'Bottle' ? 'selected' : '' }}>Bottle (Syrup/Liquid)</option>
                                    <option value="Tube" {{ old('base_unit') == 'Tube' ? 'selected' : '' }}>Tube (Cream/Ointment)</option>
                                    <option value="Vial" {{ old('base_unit') == 'Vial' ? 'selected' : '' }}>Vial (Injection)</option>
                                    <option value="Sachet" {{ old('base_unit') == 'Sachet' ? 'selected' : '' }}>Sachet (Powder/ORS)</option>
                                    <option value="Piece" {{ old('base_unit') == 'Piece' ? 'selected' : '' }}>Piece / Item</option>
                                </select>
                            ...
                            <div>
                                ...
                                <x-text-input id="purchase_price" name="purchase_price" type="number" step="0.01" min="0" class="mt-1 block w-full" :value="old('purchase_price', '0.00')" oninput="document.getElementById('selling_price').min = this.value" />
                                <x-input-error :messages="$errors->get('purchase_price')" class="mt-2" />
                            </div>

                            <div>
                                <x-input-label for="selling_price" :value="__('Retail Price / Base Unit (') . setting('currency_symbol', 'UGX') . ')'" />
                                <x-text-input id="selling_price" name="selling_price" type="number" step="0.01" min="0" class="mt-1 block w-full text-emerald-700 font-bold" :value="old('selling_price', '0.00')" required />
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div class="mb-4">
                                ...
                                <x-text-input id="reorder_level" name="reorder_level" type="number" class="mt-1 block w-full" :value="old('reorder_level', 10)" />
                                <x-input-error :messages="$errors->get('reorder_level')" class="mt-2" />
                            </div>

                            <div class="mb-4">
                                <x-input-label for="requires_prescription" :value="__('Requires Prescription')" />
                                <label class="inline-flex items-center mt-2">
                                    <input type="checkbox" name="requires_prescription" value="1" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500">
