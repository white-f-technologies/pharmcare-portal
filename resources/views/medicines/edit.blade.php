<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-2xl border border-gray-100">
                <div class="p-6">
                    <form action="{{ route('medicines.update', $medicine) }}" method="POST" enctype="multipart/form-data" x-data="medicineEditForm()">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="mb-4">
                                ...
                                <x-text-input id="name" name="name" type="text" class="mt-1 block w-full font-semibold text-gray-800" :value="old('name', $medicine->name)" required />
                            ...
                            <div class="mb-4">
                                ...
                                <input id="manufacturer" name="manufacturer" type="text" list="manufacturers-list" class="mt-1 block w-full border-gray-300 focus:border-emerald-500 focus:ring-emerald-500 rounded-md shadow-sm text-sm" value="{{ old('manufacturer', $medicine->manufacturer) }}" placeholder="Type or select manufacturer..." />
                        ...
                        <div class="mb-5 bg-slate-50 p-5 rounded-2xl border border-slate-200">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                                <div>
                                    ...
                                    <p class="text-[11px] text-gray-500 mt-1">Smallest countable unit stored in stock.</p>
                        ...
                        <div class="mb-4">
                            ...
                            @if($medicine->image_url)
                                <div class="mb-2 flex items-center gap-3">
                                    <img src="{{ $medicine->image_url }}" alt="{{ $medicine->name }}" class="w-16 h-16 object-contain rounded-xl border border-gray-200 bg-gray-50 p-1">
                                    <span class="text-xs text-gray-500 font-medium">{{ __('Current Image') }}</span>
                                </div>
                            @endif
                        ...
                        <div class="mb-4">
                            ...
                            <textarea id="description" name="description" rows="3" class="mt-1 block w-full border-gray-300 focus:border-emerald-500 focus:ring-emerald-500 rounded-md shadow-sm text-sm" placeholder="Indications, dosage warnings, or storage instructions...">{{ old('description', $medicine->description) }}</textarea>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 bg-gray-50 p-4 rounded-xl border border-gray-200">
                            <div>
                                ...
                                <x-text-input id="reorder_level" name="reorder_level" type="number" min="0" class="mt-1 block w-full" :value="old('reorder_level', $medicine->reorder_level)" />
                                <x-input-error :messages="$errors->get('reorder_level')" class="mt-2" />
                            </div>

                            <div>
                                <x-input-label :value="__('Prescription Requirement')" />
                                <label class="inline-flex items-center mt-3 cursor-pointer">
                                    <input type="checkbox" name="requires_prescription" value="1" class="rounded border-gray-300 text-emerald-600 shadow-sm focus:ring-emerald-500" {{ old('requires_prescription', $medicine->requires_prescription) ? 'checked' : '' }}>
                            ...
                            <div>
                                ...
                                <label class="inline-flex items-center mt-3 cursor-pointer">
                                    <input type="checkbox" name="is_active" value="1" class="rounded border-gray-300 text-emerald-600 shadow-sm focus:ring-emerald-500" {{ old('is_active', $medicine->is_active) ? 'checked' : '' }}>
                        ...
                        <div class="flex items-center gap-4 mt-6">
                            <x-primary-button class="bg-emerald-600 hover:bg-emerald-700">{{ __('Update Medicine') }}</x-primary-button>
    ...
    <script>
        function medicineEditForm() {
            return {
                units: @json(old('units', $units ?? [])),
                base_unit: @json(old('base_unit', $medicine->base_unit ?? 'Tablet')),
                purchase_price: @json(old('purchase_price', $purchasePrice)),
                selling_price: @json(old('selling_price', $sellingPrice)),
