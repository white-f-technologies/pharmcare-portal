    ...
    <body class="font-sans antialiased bg-slate-50 text-gray-900">
        <div class="min-h-screen bg-slate-50">
            ...
            @if(!empty($licenseWarning))
                <div class="bg-amber-50 border-b border-amber-200" x-data="{ show: true }" x-show="show" x-transition>
                    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2.5 flex items-center justify-between gap-4">
                        <div class="flex items-center gap-2 text-amber-800">
                            ...
                            <p class="text-xs font-semibold">{{ $licenseWarning }}</p>
                        </div>
                        <button @click="show = false" class="text-amber-600 hover:text-amber-800 transition">
