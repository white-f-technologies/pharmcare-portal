    ...
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-sm text-gray-500 mt-0.5">{{ __('Update personal information, change access role, or reset password.') }}</p>
            </div>
        </div>
    </x-slot>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8">
                <form method="POST" action="{{ route('users.update', $user) }}" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $user->name) }}" required class="w-full rounded-xl border border-gray-200 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 text-sm">
                        @error('name')
                    ...
                    <div>
                        ...
                        <input type="email" name="email" id="email" value="{{ old('email', $user->email) }}" required class="w-full rounded-xl border border-gray-200 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 text-sm">
                        @error('email')
                            <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="phone" id="phone" value="{{ old('phone', $user->phone) }}" class="w-full rounded-xl border border-gray-200 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 text-sm">
                        @error('phone')
                            <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <!-- Role Selection -->
                    <div>
                        <label for="role" class="block text-sm font-semibold text-gray-700 mb-2">{{ __('System Role') }} *</label>
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <label class="relative flex flex-col p-4 border border-gray-200 rounded-xl cursor-pointer hover:border-emerald-500 has-[:checked]:border-emerald-600 has-[:checked]:bg-emerald-50/40 transition">
                                <input type="radio" name="role" value="pharmacist" {{ old('role', $user->role) === 'pharmacist' ? 'checked' : '' }} class="sr-only">
                                <span class="font-bold text-sm text-gray-900 flex items-center gap-1.5 mb-1">
                            ...
                            <label class="relative flex flex-col p-4 border border-gray-200 rounded-xl cursor-pointer hover:border-emerald-500 has-[:checked]:border-emerald-600 has-[:checked]:bg-emerald-50/40 transition">
                                <input type="radio" name="role" value="cashier" {{ old('role', $user->role) === 'cashier' ? 'checked' : '' }} class="sr-only">
                                <span class="font-bold text-sm text-gray-900 flex items-center gap-1.5 mb-1">
                    ...
                    <div class="pt-4 border-t border-gray-100 space-y-4">
                        <div>
                            <h3 class="text-sm font-bold text-gray-800">{{ __('Reset Password (Optional)') }}</h3>
                            <p class="text-xs text-gray-500">{{ __('Leave password fields blank if you do not wish to change the password.') }}</p>
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label for="password" class="block text-sm font-semibold text-gray-700 mb-1">{{ __('New Password') }}</label>
                            ...
                            <div>
                                <label for="password_confirmation" class="block text-sm font-semibold text-gray-700 mb-1">{{ __('Confirm New Password') }}</label>
                    ...
                    <div class="pt-4 border-t border-gray-100">
                        <label class="inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="is_active" value="1" {{ old('is_active', $user->is_active) ? 'checked' : '' }} class="rounded border-gray-300 text-emerald-600 shadow-sm focus:ring-emerald-500 w-4 h-4">
                            <span class="ms-2 text-sm font-semibold text-gray-700">{{ __('Account is active') }}</span>
                        </label>
                    </div>
