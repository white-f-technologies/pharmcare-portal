<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="font-bold text-2xl text-gray-800 leading-tight">{{ __('Edit Customer Account') }}: {{ $customer->name }}</h2>
                <p class="text-xs text-gray-500 mt-0.5">{{ __('Update customer contact details or change active status.') }}</p>
            </div>
        </div>
    </x-slot>
                ...
                <form action="{{ route('customers.update', $customer) }}" method="POST" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <x-text-input id="name" name="name" type="text" class="mt-1.5 block w-full py-2.5 px-4 rounded-xl text-base" :value="old('name', $customer->name)" required />
                            <x-input-error :messages="$errors->get('name')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="phone" :value="__('Phone Number')" class="font-semibold text-sm" />
                            <x-text-input id="phone" name="phone" type="text" class="mt-1.5 block w-full py-2.5 px-4 rounded-xl text-base" :value="old('phone', $customer->phone)" required />
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <x-text-input id="email" name="email" type="email" class="mt-1.5 block w-full py-2.5 px-4 rounded-xl text-base" :value="old('email', $customer->email)" />
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <label class="relative flex items-center p-3.5 border border-gray-200 rounded-xl cursor-pointer hover:border-emerald-500 has-[:checked]:border-emerald-600 has-[:checked]:bg-emerald-50/50 transition">
                                    <input type="radio" name="is_active" value="1" {{ old('is_active', $customer->is_active) ? 'checked' : '' }} class="w-4 h-4 text-emerald-600 focus:ring-emerald-500 border-gray-300">
                                ...
                                <label class="relative flex items-center p-3.5 border border-gray-200 rounded-xl cursor-pointer hover:border-red-500 has-[:checked]:border-red-600 has-[:checked]:bg-red-50/50 transition">
                                    <input type="radio" name="is_active" value="0" {{ !old('is_active', $customer->is_active) ? 'checked' : '' }} class="w-4 h-4 text-red-600 focus:ring-red-500 border-gray-300">
                                    <div class="ms-3 flex items-center gap-1.5">
                    ...
                    <div>
                        ...
                        <textarea id="address" name="address" rows="3" class="mt-1.5 block w-full border-gray-300 focus:border-emerald-500 focus:ring-emerald-500 rounded-xl shadow-sm py-2.5 px-4 text-base focus:outline-none transition">{{ old('address', $customer->address) }}</textarea>
